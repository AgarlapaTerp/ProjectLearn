...
OUTPUT_SLIDES_DIR = f"./static/videos"

FRAME_RATE = 3       #5           # no.of frames per second that needs to be processed, fewer the count faster the speed
WARMUP = FRAME_RATE              # initial number of frames to be skipped
FGBG_HISTORY = FRAME_RATE * 15 #6  # no.of frames in background object
...
MIN_PERCENT = 0.1   #0.2             # min % of diff between foreground and background to detect if motion has stopped
MAX_PERCENT = 3   #0.6               # max % of diff between foreground and background to detect if frame is still in motion
